...
$smtpToEmailRaw = (string) ($SMTP_TO_EMAIL ?? getenv('SMTP_TO_EMAIL') ?? $defaultSmtpEmail);
$smtpToEmails = preg_split('/[,\s;]+/', $smtpToEmailRaw) ?: [];
$smtpToEmails = array_values(array_unique(array_filter(array_map('trim', $smtpToEmails), static function ($recipient) {
    return filter_var($recipient, FILTER_VALIDATE_EMAIL) !== false;
})));
if (empty($smtpToEmails)) {
    $smtpToEmails = [$defaultSmtpEmail];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($isAgendaForm) {
        ...
        $dayOfWeek = (int) $fechaObj->format('w'); // 0 domingo, 6 sabado

        if ($dayOfWeek === 0) {
            redirect('contacto.php?error=9' . $redirectHash);
        }

        $allowedHours = $dayOfWeek === 6 ? $saturdayHours : $weekdayHours;
        if (!in_array($horaCitaRaw, $allowedHours, true)) {
            redirect('contacto.php?error=9' . $redirectHash);
        }
    ...
    $internalSubject = "Nuevo lead web - {$nombreMail} [{$langTag}]";
            ...
            foreach ($smtpToEmails as $recipientEmail) {
